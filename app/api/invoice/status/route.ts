...
const PAYMENT_AMOUNT = '0.00025' // 0.00025 ETH (native ETH, not a token)
...
async function verifyViaRPC(
  ...
  try {
    ...
    return NextResponse.json({
      status: 'pending',
      invoiceId,
      message: 'Payment verification is checking your transaction. If you just sent the payment, please wait 10-30 seconds for it to be confirmed on-chain, then try again. Make sure you sent exactly 0.00025 ETH from your connected wallet to the recipient address on Ethereum mainnet.',
    })
  } catch (error: any) {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({
        status: 'pending',
        invoiceId,
        message: 'No matching payment transaction found. Please ensure you have sent exactly 0.00025 ETH from your connected wallet to the recipient address on Ethereum mainnet.',
      })
        ...
        return NextResponse.json({
          status: 'pending',
          invoiceId,
          message: `Payment verification error: ${innerError.message || 'Unknown error'}. Please try again in a moment. If the problem persists, ensure you sent exactly 0.00025 ETH from your connected wallet to the recipient address on Ethereum mainnet.`,
        })
      }
    } catch (error: any) {
